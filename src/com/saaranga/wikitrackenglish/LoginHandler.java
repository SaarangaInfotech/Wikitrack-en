...
/**
 * This class handles the login action to wikipedia user account. First second
 * requests are for logging in to the user account - third request is to get the
 * rss feed
 * 
 * @author supreeth
 * 
...
//			}
	...
	/**
	 * Send a http POST request to media wiki (Ref the post message should be
	 * sent to the wikipedia: ref:http://www.mediawiki.org/wiki/API:Login)
	 * 
	 * 
	 * @param unm
	 * @param pwd
	...
	private void postFirstRequest(String unm, String pwd,
		...
		mlog("First http request for feed");
		
		URI uri = null;
		try {
			StringBuilder stringBuilder = new StringBuilder();
			stringBuilder
					.append(Constants.MYWATCHLIST_LOGIN_URL);
	...
	/**
	 * Sent the POST request again with the current session token sent as a
	...
	private String postSecondRequest(String unm, String pwd,
			HttpClient httpClient, HttpContext localContext) {
		mlog("Second http request for feed");
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder
				.append(Constants.MYWATCHLIST_LOGIN_URL);
		stringBuilder.append(unm);
	...
	/**
	 * If the token is accepted, request for the rss feed in this third request
	 * 
	 * 
	 * @param httpClient
	 * @param localContext
	...
	private void postThirdRequest(HttpClient httpClient,
		...
		mlog("Third http request for feed");
		
		URI uriwl = null;
		try {
			uriwl = new URI(Constants.MYWATCHLIST_FEED_URL);
		} catch (URISyntaxException e) {
		...
		try {
			...
			mlog("Third request response:"+feed);
