...
public class FeedDBAdapter {
	...
	private static class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			...
			try {
				...
				// .d(tag, "Three database tables are created");
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			// Whent the database scheme is to be changed just drop the old
			// database tables and call on create to create new

			db.execSQL("DROP TABLE IF EXISTS " + Constants.RECENT_CHANGES);
			db.execSQL("DROP TABLE IF EXISTS " + Constants.MY_CONTRIBUTIONS);
			db.execSQL("DROP TABLE IF EXISTS " + Constants.MYWATCHLIST);
	...
	public ArrayList<String> getAuthorList(String table_name) {
		// ////Log.e(tag,.i(tag, "getAuthorList is called ");
		open();
		Cursor cur = getFeedItems(table_name);
	...
	public boolean setPressed(String tableName, int position) {
		...
		try {
			db.execSQL("UPDATE " + tableName
					+ " SET markpressed=1 WHERE latest_index=" + position);
		} catch (Exception e) {
			e.printStackTrace();
			return false;
		} finally {
	...
	public boolean markRead(String tableName, int position) {
		...
		try {
			db.execSQL("UPDATE " + tableName
					+ " SET markread=1 WHERE latest_index =" + position);
		} catch (Exception e) {
			e.printStackTrace();
			return false;
		} finally {
	...
	public boolean markFavorite(String tableName, int position) {
		...
		try {
			db.execSQL("UPDATE " + tableName
					+ " SET favorite =1 WHERE latest_index=" + position);
		} catch (Exception e) {
			e.printStackTrace();
			return false;
		} finally {
	...
	public boolean markUnFavorite(String tableName, int position) {
		...
		try {
			db.execSQL("UPDATE " + tableName
					+ " SET favorite =0 WHERE latest_index=" + position);
		} catch (Exception e) {
			e.printStackTrace();
			return false;
		} finally {
	...
	/**
	 * Trim the database table to the number of items specified int the shared
	 * preferences
	 * 
	 * @param tableName
	 * @return
